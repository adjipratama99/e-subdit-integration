...
function ContentUpdate({ data, onClose }: { data: Personel; onClose: React.Dispatch<React.SetStateAction<boolean>> }): React.JSX.Element {
 ...
 const {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      const params = {
        ...
        ...((certifiedFiles && certifiedFiles.length) && { certified: certifiedFiles }),
      }
      ...
      if((value.hasSkep && skepFiles.length === 0) && (data.skep_urls && data.skep_urls.length === 0)) {
        toast.error("Mohon upload file SKEP")
        setLoading(false)
        return
      }

      if((value.hasCertified && certifiedFiles.length === 0) && (data.certified_urls && data.certified_urls.length === 0)) {
  ...
  const addPendidikan = () => {
    const current: Pendidikan[] = (form.state.values.pendidikan as Pendidikan[]) || []
    form.setFieldValue('pendidikan', [
      ...current,
      { id: null, jenis: "", nama_sekolah: "", tahun_mulai: "", tahun_selesai: "" }
    ])
  }
  ...
  const removePendidikan = async (idx: number) => {
    ...
    if(response?.code === 0) {
      const newPendidikan = current.filter((_: unknown, i: number) => i !== idx)
            ...
            skepField.state.value && (
              <div className="flex flex-col gap-4">
                ...
                <div className="col-span-2">
                  ...
                  {skepError && <p className="text-sm text-red-500">{skepError}</p>}
                    ...
                    <ul className="list-disc pl-5 text-sm space-y-1">
                      {skepFiles.map((f, i) => (
                        <li key={i} className="flex items-center gap-2">
                          ...
                          <Button size="sm" variant="secondary" onClick={() => skepRemoveAt(i)}>
                            Hapus
                          </Button>
                        </li>
                      ))}
            ...
            sertifikasiField.state.value && (
              <div className="flex flex-col gap-4">
                ...
                <div className="col-span-2">
                  ...
                  {certifiedError && <p className="text-sm text-red-500">{certifiedError}</p>}
                  ...
                  {certifiedFiles.length > 0 && (
                    <ul className="list-disc pl-5 text-sm space-y-1">
                      {certifiedFiles.map((f, i) => (
                        <li key={i} className="flex items-center gap-2">
                          <span>{f.name} • {(f.size / (1024 * 1024)).toFixed(2)} MB</span>
                          <Button size="sm" variant="secondary" onClick={() => certifiedRemoveAt(i)}>
                            Hapus
                          </Button>
                        </li>
                      ))}
              ...
              <div className="flex flex-col gap-4">
                ...
                <ul className="list-disc pl-5 text-sm space-y-1">
                  {data.skep_urls.map((url, i) => (
                    <li key={i} className="flex items-center gap-2">
                      <a href={url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline text-ellipsis flex items-center gap-2">
                        <FaEye /> Lihat SKEP
                      </a>
                      <Confirmation
                        trigger={<Button size="sm" variant="outline">Hapus</Button>}
                        message="Apakah Anda yakin ingin menghapus SKEP ini?"
                        onConfirm={() => handleDeleteFile((data.skep_urls.map(url => `${data.nrp}/${ url.split('/')[url.split('/').length - 1] }`) || []), 'skep')}
                      />
                    </li>
                  ))}
                </ul>
...

{
                  ...
                  {data.certified_urls.map((url, i) => (
                    <li key={i} className="flex items-center gap-2">
                      ...
                      <Confirmation
                        ...
                        onConfirm={() => handleDeleteFile((data.certified_urls.map(url => `${data.nrp}/${ url.split('/')[url.split('/').length - 1] }`) || []), 'certified')}
                      />
                    </li>
                  ))}
                </ul>
      ...
      <fieldset className="border rounded-md p-4">
        ...
        {pendidikanField.state.value?.map((_, idx) => (
          <div key={`pendidikan_${idx}`} className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 rounded-2xl border mb-4">
            <div className="flex flex-col gap-2">
              <Label value="Nama Sekolah" isRequired />
              <Input
                placeholder="Masukkan Nama Sekolah ..."
                onChange={(e) => {
                  const newPendidikan = [...(pendidikanField.state.value || [])]
                  newPendidikan[idx] = { ...newPendidikan[idx], nama_sekolah: e.target.value }
                  form.setFieldValue('pendidikan', newPendidikan)
                }}
                required
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  const newPendidikan = [...(pendidikanField.state.value || [])]
                  newPendidikan[idx] = { ...newPendidikan[idx], tahun_mulai: e.target.value }
                  form.setFieldValue('pendidikan', newPendidikan)
                }}
                required
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  const newPendidikan = [...(pendidikanField.state.value || [])]
                  newPendidikan[idx] = { ...newPendidikan[idx], tahun_selesai: e.target.value }
                  form.setFieldValue('pendidikan', newPendidikan)
                }}
                required
                value={pendidikanField.state.value?.[idx]?.tahun_selesai || ""}
              />
            </div>
              ...
              >
                ...
                <SelectContent>
                  {JenisPendidikan.map(j => (
                    <SelectItem key={j.value} value={j.value}>{j.text}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
